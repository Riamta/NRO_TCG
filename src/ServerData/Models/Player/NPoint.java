...
public class NPoint {
    ...
    private void setPointWhenWearClothes() {
        ...
        // }
        // if (this.player.rewardBlackBall.timeOutOfDateReward[6] >
        // System.currentTimeMillis()) {
        ...
        if (this.player.isPl() && this.player.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2) {
            ...
            for (Item item : this.player.inventory.itemsBag) {
                if (item.isNotNullItem() && item.template.id == 454 || item.isNotNullItem() && item.template.id == 921
                        || item.isNotNullItem() && item.template.id == 1129
                        || item.isNotNullItem() && item.template.id == 1165) {
                    for (Item.ItemOption io : item.itemOptions) {
                        switch (io.optionTemplate.id) {
                            case 14: // Chí mạng+#%
                                this.critAdd += io.param;
    ...
    private void setHpMax() {
        ...
        // set mộc ấn
        if (this.player.setClothes.mocAn == 5) {
            this.hpMax += ((long) this.hpMax * 10 / 100);
        }
        // ngọc rồng đen 2 sao: 10% HP
        if (this.player.rewardBlackBall.timeOutOfDateReward[1] > System.currentTimeMillis()) {
            this.hpMax += ((long) this.hpMax * NgocRongSaoDenService.R2S_1 / 100);
        }
        // khỉ
        ...
        if (this.player.effectSkill.isBienHinh) {
            this.hpMax += ((long) this.hpMax * (this.player.effectSkill.levelBienHinh * 5) / 100);
        }
        // ngọc rồng toribo
        ...
        // nóng
        if (this.player.itemTime.isNong) {
            this.hpMax -= (long) this.hpMax * 20 / 100;
        }
        // mía khổng lồ
        if (this.player.itemTime != null && this.player.itemTime.isUseMiaKhongLo) {
            this.hpMax += (long) this.hpMax * 10 / 100;
        }
        // mía thơm
        if (this.player.itemTime != null && this.player.itemTime.isUseMiaThom) {
            this.hpMax += (long) this.hpMax * 10 / 100;
        }
        // mía sầu riêng
        if (this.player.itemTime != null && this.player.itemTime.isUseMiaSauRieng) {
            this.hpMax += (long) this.hpMax * 10 / 100;
        }
    }
    ...
    private void setMpMax() {
        ...
        if (this.player.setClothes.picolo == 5) {
            this.mpMax += ((long) this.mpMax * 80 / 100);
        }
        // set ki 1
        ...
        // biến hình
        if (this.player.effectSkill.isBienHinh) {
            this.mpMax += ((long) this.mpMax * (this.player.effectSkill.levelBienHinh * 5) / 100);
        }
        // ngọc rồng toribo
    ...
    private void setDame() {
        ...
        if (this.player.rewardBlackBall.timeOutOfDateReward[0] > System.currentTimeMillis()) {
            this.dame += ((long) this.dame * NgocRongSaoDenService.R1S_1 / 100);
        }
        // set hỏa ấn
        if (this.player.setClothes.hoaAn == 5) {
            this.dame += ((long) this.dame * 10 / 100);
        }
        // biến hình
        if (this.player.effectSkill.isBienHinh) {
            this.dame += ((long) this.dame * (this.player.effectSkill.levelBienHinh * 5) / 100);
        }
        // khỉ
        if (this.player.effectSkill.isMonkey) {
            if (!this.player.isPet || (this.player.isPet
    ...
    private void setDef() {
        ...
        // ngọc rồng đen 2 sao
        // if (this.player.rewardBlackBall.timeOutOfDateReward[1] >
        // System.currentTimeMillis()) {
    ...
    public double getDameAttack(boolean isAttackMob) {
        ...
        // này là cộng mấy cái nội tại mình có ấy ví dụ galick +20% thì nó là :<
        // dameAttack = dameAttack * 20 / 100 >
    ...
    public long calSucManhTiemNang(long tiemNang) {
        if (power < getPowerLimit()) {
            ...
            if ((this.player.zone.map.mapId >= 122 && this.player.zone.map.mapId <= 124 && this.power < 100000000000L)
                    || (this.player.zone.map.mapId >= 204 && this.player.zone.map.mapId <= 206
                            && this.power < 20000000000L)) {
                tiemNang += tn * 2;
            }
            if (this.player.id == 11025) {
                tiemNang *= Manager.RATE_EXP_SERVER * 10;
            } else if (this.player.nPoint.power > 60000000000L) {
                tiemNang *= 1;
            } else if (this.player.nPoint.power > 50000000000L) {
                tiemNang *= Manager.RATE_EXP_SERVER / 6;
            } else if (this.player.nPoint.power > 40000000000L) {
                tiemNang *= Manager.RATE_EXP_SERVER / 5;
            } else if (this.player.nPoint.power > 30000000000L) {
