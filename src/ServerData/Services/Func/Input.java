...
import ServerData.Models.upload.update;
...
public class Input {
    ...
    public static final int ITEM_BOT = -503;
    public static final int DOI_CONG_VIEC = -502;
    ...
    public void doInput(Player player, Message msg) {
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case CHATALL:
                            String chat = text[0];
                            Service.gI().ChatAll(22713,"|7|[ THÔNG BÁO ]"+"\n" 
...
//                    break;
                ...
                case QUY_DOI_THOI_VANG:
                    ...
                    if (player.getSession().vnd >= tinhtientv) {
...
//                                + " đổi " + goldTrade + " thỏi vàng " + " " + "bạn cần thêm" +(player.getSession().vnd-goldTrade));
                ...
                case QUY_DOI_HONG_NGOC:
                    ...
                    if (player.session.vnd >= tinhtienhn) {
                   ...
                   short soxubacdoi = Short.parseShort(text[0]);
                    ...
                    if (player.getSession().vnd >= tinhtienxubac) {
...
//                                + " đổi " + goldTrade + " thỏi vàng " + " " + "bạn cần thêm" +(player.getSession().vnd-goldTrade));
                   ...
                   short sothangtinhthachdoi = Short.parseShort(text[0]);
                    ...
                    if (player.getSession().vnd >= tinhtienthangtinhthach) {
...
//                                + " đổi " + goldTrade + " thỏi vàng " + " " + "bạn cần thêm" +(player.getSession().vnd-goldTrade));
                   ...
                   short soxuvangdoi = Short.parseShort(text[0]);
                    ...
                    if (player.getSession().vnd >= tinhtienxuvang) {
...
//                                + " đổi " + goldTrade + " thỏi vàng " + " " + "bạn cần thêm" +(player.getSession().vnd-goldTrade));
 ...
 public void taobot(Player pl) {
    ...
    public void ChatAll(Player pl) {
        createForm(pl, CHATALL, "CHAT ALL PLAYER", new SubInput("Chat", ANY));
    }
   ...
   public void thayevent(Player pl) {
    ...
    public void createFormQDTV(Player pl) {
     ...
     public void createFormQDXUBAC(Player pl) {
        createForm(pl, QUY_DOI_XU_BAC, "Quy Đổi Thỏi Đồng, giới hạn đổi không quá 1000 và không nhỏ hơn 5",
                new SubInput("(VD : NHẬP VÀO 1 = 1K VND = " + Manager.TLDOIXUBAC + " Thỏi Đồng)", NUMERIC));
    }
      ...
      public void createFormQDXUVANG(Player pl) {
        createForm(pl, QUY_DOI_XU_VANG, "Quy Đổi Thỏi Bạc, giới hạn đổi không quá 1000 và không nhỏ hơn 5",
                new SubInput("(VD : NHẬP VÀO 1 = 1K VND = " + Manager.TLDOIXUVANG + " Thỏi Bạc)", NUMERIC));
    }
    ...
    public static class SubInput {
        ...
        try {
            ...
            Service.gI().sendThongBao(player, "Đã Nấu Bánh Chưng");
    ...
    private void nauBanhTet(Player player, String s) {
        try {
            ...
            Service.gI().sendThongBao(player, "Đã Nấu Bánh Tét");
            if (!NauBanh.gI().ListPlNauBanh.contains(player)) {
            NauBanh.gI().addListPlNauBanh(player);
            }
        } catch (NumberFormatException e) {
