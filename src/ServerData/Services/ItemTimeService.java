...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseMiaKhongLo) {
            sendItemTime(player, 13462, (int) ((TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMiaKhongLo)) / 1000));
        }
        if (player.itemTime.isUseMiaThom) {
            sendItemTime(player, 13463, (int) ((TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMiaThom)) / 1000));
        }
        if (player.itemTime.isUseMiaSauRieng) {
            sendItemTime(player, 13464, (int) ((TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMiaSauRieng)) / 1000));
        }
        if (player.itemTime.isNong) {
            ItemTimeService.gI().sendTextTime(player, TEXT_NONG, "Cơn Nóng Tràn Về Làm Giảm 20% Chỉ Số, Hãy Mua Nước Mía Giải Khát", (int) ((TIME_NONG - (System.currentTimeMillis() - player.itemTime.lastTimeNong)) / 1000));
            //sendItemTime(player, 30016, (int) ((TIME_NONG - (System.currentTimeMillis() - player.itemTime.lastTimeNong)) / 1000));
        }
    }
